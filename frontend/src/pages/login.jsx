...
import { useNavigate } from "react-router-dom"; // 1. Use Router for SPA navigation
...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // 3. Store both token AND user object
      // This saves you from having to decode the JWT every time you need the username
      localStorage.setItem("token", data.token);
      localStorage.setItem("user", JSON.stringify(data.user));

      // 4. Instant SPA navigation (No page reload)
      navigate("/home");
    } catch (err) {
  ...
  return (
    <div
      ...
      style={{
        background: "radial-gradient(circle at top, rgba(0,255,156,0.05), transparent 60%), #0b141a",
      }}
    >
      <div className="w-full max-w-md bg-[#111b21] rounded-2xl border border-white/10 shadow-[0_0_40px_rgba(0,255,156,0.15)] p-8 relative">
        ...
        <div className="relative mb-8 text-center">
          ...
          <p className="text-sm text-gray-400 mt-2">Secure system access</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5 relative">
          ...
          <div>
            <label className="block text-xs font-mono tracking-widest text-gray-400 mb-2">PASSWORD</label>
            <input
              type="password"
              required
              value={password}
          ...
          <button
            ...
            className={`w-full mt-2 py-3 rounded-lg text-base font-medium tracking-wide transition ${
              loading ? "opacity-70 cursor-wait" : "bg-[#00a884] hover:brightness-110 shadow-[0_0_16px_rgba(0,255,156,0.35)]"
            } text-black`}
          >
        ...
        <p className="relative text-center text-xs text-gray-500 mt-8">Encrypted • Authenticated • Secure</p>
      </div>
    </div>
  );
}